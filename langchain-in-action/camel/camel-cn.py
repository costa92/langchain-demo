...
class CAMELAgent:
  ...
  def update_messages(self, message: HumanMessage) -> list[BaseMessage]:
    """更新对话消息"""
...
task_specify_agent = CAMELAgent(
   ...
   ChatOpenAI(model_name = model_name, base_url=base_url, temperature=1.0)
)
...
assistant_agent = CAMELAgent(
   assistant_sys_msg, 
   ChatOpenAI(temperature=0.2,base_url=base_url,model_name=model_name),
)
# 创建用户CAMELAgent实例
user_agent = CAMELAgent(user_sys_msg, ChatOpenAI(temperature=0.2,base_url=base_url,model_name=model_name))
